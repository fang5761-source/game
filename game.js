...
const TICK_INTERVAL = 300; // 降低刷新頻率，較穩定
let timeRemaining = selectedTimeLimit;

// 禁用音效，避免 AudioContext 造成瀏覽器當機
...
function endGameSummary() {
    ...
    let summaryHtml = `
        <p class="text-xl font-black text-purple-800 mb-1">🎉 回合總結 🎉</p>
        <p class="text-sm font-semibold text-gray-700">總題數: ${total} 題  |  答對題數: ${correct} 題</p>
        <p class="text-3xl font-extrabold text-red-600 mt-2">${accuracy.toFixed(0)}%</p>
    `;
    let encouragement;

    if (isPerfect) {
        encouragement = `<div class="text-3xl font-black text-yellow-600 mt-3 clap-animation">👏 完美全對！太棒了！ 🏆</div>`;
    } else {
        encouragement = `<div class="text-xl font-black text-red-600 mt-3">💪 挑戰結束！再接再厲，加油！</div>`;
    }
